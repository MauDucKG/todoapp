...
function App() {
  ...
  useEffect(() => {
    for (let i = 0; i < 3; i++)
      axios
        .get(
          "https://localhost:7260/api/ToDoList?filterOn=status&filterText=" + i,
          {
            headers: {
  ...
  function handleCancelNew(id, task) {
    setPrimitiveTasks((prevTasks) => {
      ...
      if (index !== -1) {
        // Check if object with matching id is found
        const updatedContent = updatedTasks[index].content.filter(
    ...
    if (active.data.current?.type === "singleTask")
      setPrimitiveTasks((prevTasks) => {
        ...
        const activeIndex = updatedTasks.findIndex(
          (task) => task.id == active.data.current.task.status
        );
        const id =
          over.data.current?.type === "singleTask"
            ? over.data.current.task.status
            : over.id;
        const overIndex = updatedTasks.findIndex((task) => task.id == id);
        if (overIndex === activeIndex) {
          const index = updatedTasks[activeIndex].content.findIndex(
            (x) => x.id == active.id
          );
          const overActiveColumnIndex = updatedTasks[
            activeIndex
          ].content.findIndex((x) => x.id == over.id);
          if (overActiveColumnIndex == -1) return prevTasks;
          const updatedActiveContent = arrayMove(
            updatedTasks[activeIndex].content,
            overActiveColumnIndex,
            index
          );
          ...
          // Check if object with matching id is found
          const updatedActiveContent = updatedTasks[activeIndex].content.filter(
          ...
          axios
            .put(
              "https://localhost:7260/api/ToDoList/" +
                updatedTaskWithNewStatus.id,
              taskWithoutId,
              {
                headers: {
                  "Content-Type": "application/json",
          ...
          const updatedOverContent = [
            updatedTaskWithNewStatus,
            ...updatedTasks[overIndex].content,
          ];
          updatedTasks[activeIndex].content = updatedActiveContent;
  ...
  const columnsIDs = useMemo(
    () => primitiveTasks.map((t) => t.id.toString()),
    [primitiveTasks]
  );
  ...
  return (
    <>
      <MainHeader
        handleInfoClose={handler}
        isInfoShown={isInfoShown}
        addTask={addTask}
        setSearchTerm={handleKeywordSearch}
        ...
        <main style={{ marginTop: isInfoShown ? "15vh" : "8vh" }}>
            ...
            <SortableContext items={columnsIDs}>
              {primitiveTasks.map((task) => (
                <ToDo
                  key={task.id}
                  status={task.id}
                  updatedTasks={addOrUpdateTask}
                  tasks={getContentById(task.id)}
                  cancelTask={handleCancelNew}
          ...
          >
            {!isInfoShown && (
              <IconButton
                aria-label="Close"
                style={{ outline: "none", cursor: "hand" }}
                onClick={() => setIsInfoShown(true)}
              >
                <FaCircleInfo
                  style={{ width: "20px", height: "20px", fill: "url(#lgrad)" }}
                />
              </IconButton>
            )}
          </div>
          ...
          <DragOverlay>
            {activeColumn && (
              <ToDo
                status={activeColumn}
                updatedTasks={addOrUpdateTask}
                tasks={getContentById(activeColumn)}
                cancelTask={handleCancelNew}
              ></ToDo>
            )}
            {activeTask && (
              <CustomCard
                ...
                updatedTasks={addOrUpdateTask}
                list={getContentById(activeTask.status)}
