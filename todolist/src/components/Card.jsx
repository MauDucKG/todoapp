...
export default function CustomCard({
  ...
  if (currTask.estimatedTime.days !== 0)
    et += currTask.estimatedTime.days + " day ";
  if (currTask.estimatedTime.hours !== 0)
    et += currTask.estimatedTime.hours + " hour";
  ...
  function deleteTask(taskId) {
    axios
      .delete(`https://localhost:7260/api/ToDoList/${taskId}`, {
        headers: {
      ...
      .then((response) => {
        if (response.status === 200) {
          ...
          // Cập nhật danh sách nhiệm vụ sau khi xóa
      ...
      .catch((error) => {
        ...
        // Gọi hàm handleTokenRefresh nếu cần thiết
        handleTokenRefresh();
      });
  }
  ...
  return (
    <>
      {!isEditing && task.id !== "new" && (
        ...
        >
          <div className={styles.edit} onClick={handleExpand}>
            <FaEdit />
          </div>
          <div className={styles.delete} onClick={() => deleteTask(task.id)}>
            <MdDelete />
          </div>
          ...
          <CardContent sx={{ padding: "2px" }}>
            <Grid container spacing={2} className={styles.grid}>
              ...
              <Grid item xs={7} className={styles.priority}>
                <Typography
                  variant="body1"
                  sx={{ bgcolor: colors[getPriorityColor()] }}
                >
                  {getPriorityColor()}
                </Typography>
              </Grid>
      ...
      {(isEditing || task.id === "new") && (
        <div className={styles.card}>
          <div className={styles.edit} onClick={handleCancel}>
            <IoClose />
          </div>
          <TextField
            variant="standard"
          ...
          <CardContent sx={{ padding: "2px" }}>
            <Grid
              container
              spacing={2}
              className={`${styles.grid} ${styles.clicked}`}
            >
              <Grid item xs={5}>
                <Typography variant="body1">Description:</Typography>
              </Grid>
          ...
          <button
            onClick={() => {
              ...
              if (isNew) {
                axios
                  .post("https://localhost:7260/api/ToDoList", taskWithoutId, {
                    headers: {
                      "Content-Type": "application/json",
                      Authorization: `Bearer ${localStorage.getItem("token")}`,
                    },
                ...
                axios
                  .put(
                    `https://localhost:7260/api/ToDoList/${task.id}`,
                    taskWithoutId,
                    {
                      headers: {
                        "Content-Type": "application/json",
